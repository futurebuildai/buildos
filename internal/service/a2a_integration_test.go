...
func TestA2AService_ProcessWebhook_ReviewMaterialQuote(t *testing.T) {
	...
	svc := NewA2AService(pool, store.NewA2AStore(), store.NewFeedCardsStore(), nil)
...
func TestA2AService_ProcessWebhook_DefaultOrgID_Fallback(t *testing.T) {
	...
	svc := NewA2AService(pool, store.NewA2AStore(), store.NewFeedCardsStore(), &defaultOrg)
...
func TestA2AService_ProcessWebhook_RequiresOrgID(t *testing.T) {
	pool := testdb.NewPool(t)
	ctx := context.Background()
	svc := NewA2AService(pool, store.NewA2AStore(), store.NewFeedCardsStore(), nil) // no defaultOrgID
