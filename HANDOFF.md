...
**Next:** Flutter golden tests for the offline/sync visual states (`mobile/test/`
golden harness). The web live lane now covers claim→wizard→portfolio,
recalc→CPM cascade-diff, AND BYOK→AI-on capability flip (the `/capabilities`
backend gap is closed — see the top "Last shipped" entry).
