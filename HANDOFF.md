...
flag for owner sign-off before landing) OR Flutter golden tests for the
offline/sync visual states (`mobile/test/` golden harness). The web live lane
covers claim→wizard→portfolio, recalc→CPM cascade-diff, and BYOK→AI-on
capability flip. **Backup/DR is now in** (`scripts/backup-db.sh` +
`restore-db.sh` + `docs/dr-runbook.md`, NEXT_STEPS Tier 2 #5 — see the top
"Last shipped" entry). NEXT_STEPS.md has drift: Tier 1 #1–3 + Tier 2 #4–5 all
shipped, #7 (Brain mTLS) is obsolete post-pivot.
