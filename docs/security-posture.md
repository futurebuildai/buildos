...
| L-2 | Low | Missing `X-Content-Type-Options` / `Referrer-Policy` / `X-Frame-Options` | **FIXED** — `mw.SecurityHeaders` sets nosniff + `Referrer-Policy: no-referrer` + `X-Frame-Options: DENY` on every response. |
...
2. **`TRUSTED_PROXY_CIDRS`** = your LB/ingress subnet(s) *only if* an upstream you
   control terminates the connection and appends an accurate `X-Forwarded-For`.
   Leave empty if the app is reachable directly — spoofed headers are then ignored.
3. **Network-restrict `/metrics`** to the Prometheus scraper (unauth by convention).
4. Run the **k6 harness** (`scripts/k6/`) against a staging instance; the
