...
## Verification
Built grounded (read the exact `ListAssignedTasks` + `pull`/Drift patterns to mirror) off the ultraplan, whose 2-critic pass had already caught + fixed the **build-breaking delta bug** (created_at delta → permanent staleness; corrected to full-replace) before any code. (An adversarial code review is the next step if the owner runs `/code-review`.)
