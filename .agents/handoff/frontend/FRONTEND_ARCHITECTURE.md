...
| **OQ-9** | Does any `plan_tier` gating survive the standalone pivot, or is it removed entirely? If it survives, what's the no-billing-surface UX for a `402/403` on AI routes? | Treat as removed; map any tier 402/403 to the same "AI unavailable" soft notice (no upsell) | There is no billing UI to upsell into. |
